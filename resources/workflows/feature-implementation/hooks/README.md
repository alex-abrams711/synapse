...
def load_synapse_config():
    ...
    if not os.path.exists(config_path):
        print("⚠️ Synapse config not found", file=sys.stderr)
        return None
...
def parse_tasks(tasks_file_path, schema):
    ...
    try:
        ...
        print("💡 Run 'synapse sense' to regenerate schema", file=sys.stderr)
        return None
...
if not config:
    print("ℹ️ No config - allowing work", file=sys.stderr)
    sys.exit(1)  # Exit code 1 = Allow

tasks_file, schema = find_active_tasks_file(config)
if not tasks_file:
    print("ℹ️ No task management - allowing work", file=sys.stderr)
    sys.exit(1)  # Exit code 1 = Allow
```
...
try:
    ...
    print("💡 Run 'synapse sense --validate' to check schema", file=sys.stderr)
    print("💡 Run 'synapse sense --regenerate' to fix", file=sys.stderr)
...
try:
    parsed_tasks = parser.parse_tasks_file(tasks_file_path)
    if not parsed_tasks:
        print("⚠️ No tasks found", file=sys.stderr)
        sys.exit(1)  # Allow if no tasks
except Exception as e:
    print(f"❌ Parsing error: {e}", file=sys.stderr)
    print("💡 Check tasks file format", file=sys.stderr)
...
print(f"🔍 Analyzing task: {task_id}", file=sys.stderr)
print(f"✅ Blocking check passed", file=sys.stderr)
...
# Allow work
if should_allow:
    print("✅ Allowing work", file=sys.stderr)
    sys.exit(1)
...
def main():
    print("🔍 Running your-hook...", file=sys.stderr)
    ...
    print("✅ Hook passed", file=sys.stderr)
    sys.exit(1)
...
**Error**: `⚠️ Synapse config not found`
...
**Error**: `❌ Schema validation failed: Unsupported schema version`
...
**Error**: `❌ Error parsing tasks: No match for task line`
...
**Warning**: `⚠️ Schema validation below threshold: 85% match rate`
