...
- `{{workflow_dir}}/task_log.json` - Task log integrity
- `{{workflow_dir}}/workflow_state.json` - Workflow state consistency
...
**Directory Structure:**
- Required directories exist (`.claude/`, `{{workflow_dir}}/`)
...
🔧 Suggested fixes:
  ...
  2. Restore from backup: cp {{workflow_dir}}/config.yaml.backup {{workflow_dir}}/config.yaml
