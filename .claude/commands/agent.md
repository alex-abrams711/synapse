...
### Data Sources
- `.synapse/config.yaml` - Agent configurations
- `.synapse/task_log.json` - Agent activity history
- `.synapse/workflow_state.json` - Current agent assignments
...
✓ Updated custom_prompt for DEV agent
  - Configuration saved to .synapse/config.yaml
...
💡 Check file permissions on .synapse/config.yaml
   Or run: /validate --fix
```
